...
$(function(){
  ...
  var aspectRatio = 0.50;   // 0.78  1.28
  var idealWidth = 3600;
  ...
  function makeFigure (props = {x: 0, y: 0, z: 0, scale: 1}) {
    ...
    var torso = makeAttachable({
      thing: Thing.Img.make({
        ...
        w: 650 * S,
        h: 1000 * S,
        src: 'img/gold_picture_frame_vert.png',
  ...
  function makeImgArrayH(props = {imgNames: []}) {
    return props.imgNames.map((imgName /*, index */) => {
      return Thing.Img.make({
        src: imgPath + imgName,
        h: props.h,
  ...
  function makeImgArrayScroll(props) {
    ...
    // eyeImgBox.$element.attr('data-simplebar', '');
  ...
  function makeImgArrayScrollH(props) {
    var eyeImgs = makeImgArrayH({
      imgNames: props.imgNames,
      h: props.h,
    });
    var imgWrapperBox = Thing.Box.make({
      w: 7000,
      h: props.h,
    }).add(eyeImgs);
    var eyeImgBox = Thing.Box.make($.extend({
  ...
  function makeSuitColumns(props) {
    ...
    makeWidths(props).forEach(function (xw) {
      ...
      parts.push(Thing.Img.make({
        ...
        left: Math.round((xw.x/props.w) * 100) + '%',  // img position is a % of parent container (have to use 'left' property because x prop can't handle %)
        y: y,
        z: around(props.z, 10),
        w: Math.round((xw.w/props.w) * 100) + '%',  // img width is a % of parent container
        h: props.h,
  ...
  function makeScrollingEyeBoxesH(props) {
    var eyeW = props.w * 0.2;
    var eyeH = props.h * 0.2;
    var eyeRX = around(props.w * 0.35, props.w * 0.1);
    return [
      makeImgArrayScrollH({  // scrolling eyes right side
        id: 'eyes-right-scroll',
        imgNames: randomArray(imgNamesLapelsRight, 10),
        x: 100, //eyeRX,
        y: 100, //props.h * 0.10,
        z: props.z,
        w: 300, //eyeW * 10,
      ...
      makeImgArrayScrollH({  // scrolling eyes left side
        id: 'eyes-left-scroll',
        imgNames: randomArray(imgNamesTies, 10),
        x: 400, //eyeRX + eyeW,
        y: 100, //props.h * 0.14,
        z: props.z,
        w: 300, //eyeW * 10,
      ...
      makeImgArrayScrollH({  // scrolling eyes left side
        id: 'eyes-left-scroll',
        imgNames: randomArray(imgNamesLapelsLeft, 10),
        x: 700, //eyeRX + eyeW,
        y: 100, //props.h * 0.14,
        z: props.z,
        w: 400, //eyeW * 10,
  ...
  */
  function makePowerSuitPortrait (props = {w:1000, h:1500}) {
    ...
    // scrolling eyes
    bounds.add(makeScrollingEyeBoxesH($.extend({}, props, {z: baseFaceZ + 250})));
  ...
  function main() {
    ...
    stage.add(powerSuitPortrait);
    stage.add(makeFigure({
        scale: (CW/idealWidth) * 0.5,
        x: CW * 0.7,
        y: CH * 0.1,
        z: 15
      })
